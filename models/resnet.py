...
from .linear import inner_product
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        #print('bias_init_mode = {}, bias_init_mean = {}'.format(bias_init_mode, bias_init_mean))
        ...
        if not fixdim:
            ...
            self.fc = inner_product(in_features = 512 * block.expansion, out_features = num_classes, bias_init_mode = bias_init_mode, bias_init_mean = bias_init_mean)
        else:
            self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                           dilate=replace_stride_with_dilation[2], outdim=num_classes)
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
            #self.fc = nn.Linear(num_classes, num_classes, bias=fc_bias)
            self.fc = inner_product(in_features = num_classes, out_features = num_classes, bias_init_mode = bias_init_mode, bias_init_mean = bias_init_mean)
            # self.fc = inner_product_normalized(in_features = num_classes, out_features = num_classes, bias_init_mode = bias_init_mode, bias_init_mean = bias_init_mean)
