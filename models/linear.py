...
class inner_product_normalized(nn.Module):
    r"""
    """
    __constants__ = ['in_features', 'out_features']
    in_features: int
    out_features: int
    def  __init__(self, in_features = 512, out_features = 1000, bias_mode = 'diverse', bias_init_mode = 0, gamma = 96., bias_init_mean = 0.):
        super(inner_product_normalized, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.weight = Parameter(torch.FloatTensor(out_features, in_features))
        self.gamma = gamma
    ...
    def reset_parameters(self) -> None:
        ...
        if self.bias_mode == 'unified':
            nn.init.constant_(self.bias, math.log(self.gamma * self.out_features))
            if self.gamma > 10:
                nn.init.constant_(self.bias, math.log(self.gamma * self.out_features))
            else:
                nn.init.constant_(self.bias, math.log(self.gamma * self.out_features))
                #nn.init.constant_(self.bias, math.log(self.gamma))
        elif self.bias_mode == 'diverse':
            if self.bias_init_mode == 0:
                fan_in, _ = init._calculate_fan_in_and_fan_out(self.weight)
                bound = 1 / math.sqrt(fan_in)
    ...
    def forward(self, input: torch.Tensor):
        ...
        output = self.gamma * cos_theta - self.bias if self.bias is not None else self.gamma * cos_theta
        return output
